...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        total_success = spacex_df[spacex_df['class'] == 1].groupby('Launch Site').size().reset_index(name='Success Count')
        # Create pie chart for all sites
        fig = px.pie(
            total_success,
            names='Launch Site',
            values='Success Count',
            title='Count of Successful Launches for All Launch Sites')
        
    else:
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        # Create a pie chart for success and failed launches for the selected site
...
def get_scatter(selected_site, selected_payload_range):
    ...
    if selected_site == 'ALL':
        # Create a scatter plot for all values of Payload Mass and class
        fig = px.scatter(
            spacex_df,
            x='Payload Mass (kg)',
            y='class',
        ...
        # Create a scatter plot for Payload Mass and class for the selected site
        fig = px.scatter(
            filtered_df,
            x='Payload Mass (kg)',
            y='class',
